...
from tasks.tasks import AGENT_HEADER_MAP
from core.channel import GithubIssueAdapter
...
# Worker Architecture 컴포넌트 (공유 인스턴스)
_arch_repo = ArchitectureRepository(
    db_path=os.getenv("ARCHITECTURE_DB_PATH", ".agent_architecture.db"),
    backend=os.getenv("ARCHITECTURE_DB_BACKEND", "sqlite"),
    postgres_dsn=os.getenv("ARCHITECTURE_POSTGRES_DSN"),
)
_arch_orchestrator = ManagerOrchestrator(_arch_repo)
_arch_queue = create_task_queue()
_worker_runtime = WorkerRuntime(_arch_queue, _arch_orchestrator)
_github_adapter = GithubIssueAdapter(_arch_repo, _arch_orchestrator, _arch_queue)
...
def process_issue(issue_number: int, dashboard_callback=None):
    """단일 이슈를 Worker Architecture를 통해 처리.

    GithubIssueAdapter → Task Queue → Worker → Orchestrator → Workflow (CrewAI)
    ...
    gh_repo = _get_repo()
    before_count = _count_comments(gh_repo, issue_number)
    ...
    try:
        ...
        _write_system_error_comment(gh_repo, issue_number, f"이슈 로드 실패: {e}")
        raise

    # 채널 어댑터: GitHub Issue → Task + Queue 진입
    try:
        ingest_result = _github_adapter.ingest(
            issue_number=issue_number,
            title=title,
            body=body,
        )
        print(f"[채널] task_id={ingest_result.task.task_id}, job_id={ingest_result.job_id}")
    except Exception as e:
        from usage_tracking import send_discord_run_failed
        send_discord_run_failed(issue_number, str(e))
        _write_system_error_comment(gh_repo, issue_number, f"태스크 생성 오류: {e}")
        raise

    # Worker 즉시 실행 (CLI 모드: 동기 블로킹 — worker가 CrewAI 실행 완료까지 기다림)
    try:
        worker_result = _worker_runtime.run_once(timeout_seconds=2)
        if worker_result.task_id and not worker_result.ok:
            raise RuntimeError(worker_result.message)
    except Exception as e:
        from usage_tracking import send_discord_run_failed
        send_discord_run_failed(issue_number, str(e))
        _write_system_error_comment(gh_repo, issue_number, str(e))
        raise

    print(f"\n[완료] Issue #{issue_number}")
    print(f"{'='*50}")

    # 댓글 검증: 매니저 + 기본 팀 헤더가 달렸는지 확인
    default_headers = [AGENT_HEADER_MAP[aid] for aid in ("manager", "dev", "qa") if aid in AGENT_HEADER_MAP]
    after_count = _count_comments(gh_repo, issue_number)
    new_count = max(after_count - before_count, 0)
    print(f"[검증] 실행 후 댓글 수: {after_count} (신규 {new_count}개)")

    missing = _find_missing_agents(gh_repo, issue_number, before_count, default_headers)
    if missing:
        print(f"[검증] 댓글 누락 에이전트: {', '.join(missing)}. 보정 댓글을 작성합니다.")
        _force_comment(gh_repo, issue_number, missing, worker_result.message)
    else:
        print(f"[검증] 모든 에이전트({len(default_headers)}명)가 정상적으로 댓글을 작성했습니다.")
...
def run_dashboard(port: int = 3000, watch: bool = False, interval: int = 300):
    ...
    from dashboard_state import set_run_started, set_run_finished
    from dashboard.server import app, register_runner

    def process_issue_with_dashboard(issue_number: int):
        """Worker Architecture를 통해 이슈를 처리하고 대시보드 상태를 업데이트."""
